...
model_1 <- lm(log(price)~ Bedroom.AbvGr, data=data.model)
summary(model_1)
```
Now, model 2 summary.
```{r}
model_2 <- lm(log(price)~ Bedroom.AbvGr+log(area), data=data.model)
summary(model_2)
```
...
model_3 <-lm(log(price)~ log(area), data=data.model)
predict_3 <- predict(model_3, newdata=ames_train)
ames_train$resid <- resid(model_3)
ames_train %>%
...
model_4 <- lm(log(price)~ log(area), data = data.model)
predict_4 <- predict(model_4, newdata = ames_train)
ames_train$resid <- resid(model_4)
ames_train %>%
  group_by(Neighborhood) %>%
...
Solution_5 <- ames_train %>% dplyr:: select(Overall.Qual, Bsmt.Qual, Garage.Qual, price)

Solution_5.model <- Solution_5[complete.cases(Solution_5),]

print(paste('NUmber of Obs. must be deselected to estimate the model', nrow(Solution_5)-nrow(Solution_5.model)))
```
...
Solution_6 <- ames_train %>% dplyr:: select(Bsmt.Qual, Garage.Qual, price)
Solution_6.model <- Solution_6[complete.cases(Solution_6),]
Solution_6_na <- is.na(Solution_6)
```
...
Solution_7 <- lm(log(price)~ Overall.Cond + Overall.Qual, data = ames_train)
Prediction_7 = predict(Solution_7, newdata = ames_train)
ames_train$Prediction_7 = exp(Prediction_7)

ames_train %>%
  group_by(MS.SubClass) %>%
  summarise(median_price=median(Prediction_7)) %>%
...
which.max(lm.influence(Solution_7)$hat)
```
...
Solution_9 <- lm(log(price)~ Bedroom.AbvGr, ames_train)
coef(Solution_9)
```
...
Solution_10 <- lm(log(price) ~ Sale.Condition, data = ames_train)
summary(Solution_10)
```
...
print(paste('Percentage of the obs. remain', nrow(normal)/1000*1000))
```
...
Solution_12 <- lm(log(price)~ log(area), data=normal)
summary(Solution_12)$adj.r.squared
```
Now,
```{r}
