...
predict_full <- exp(predict(model.AIC, ames_test))
mean(predict_full)
```
...
sqrt(mean((exp(predict(model.bas, newdata = ames_test, estimator = "BPM")$fit)- ames_test$price)^2))
```
...
predict.test.MPM <- predict(model.bas, ames_test, estimator = "MPM", prediction=TRUE, se.fit = TRUE)
resid.MPM = ames_test$price-exp(predict.test.MPM$fit)
plot(ames_test$price, resid.MPM, xlab = "Price", ylab = "Residuals")
```
